...
def updateWeight(alpha_, old, xx, yy, target):
    for i in range(len(old)):
        old[i] += alpha_ * xx[i] * (target - yy)
    return old
...
for i in range(10):
    ...
    while True:
        ...
        for x in Train_data:
            ...
            for i in range(len(x) - 1):
                y += x[i] * weight[i]
            y += bias
            weight = updateWeight(alpha, weight, x[:-1], y, t)
            bias += alpha * (t - y)
    ...
    saveWeights("AdaLine_weights.txt", wSt)
    print("epochs: ", e)
...
s = f'Adaline ( avg: {str(sum / 10)}, max: {results[9]}) \n'
f.write(s)
f.close()
